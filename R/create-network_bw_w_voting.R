...
# /data/MicrobiomeCore/analysis/KFAN01_20220809/Transnet_mw_sp/merged/corr/C1-C4/p1_cp1_cfdr1/per_analysis/KD_TP2_voting
# usage: Rscript /data/rodriguesrr/scripts/R/create-network_bw_w_voting.R --file ../../merged_sp_FolChMedian_merged-parallel-output.csv --group KD_TP2 --consistent pre-consis-of-Analys5-pairs-of-phenos_and_otus_KD.TP2.C1-C4.consis_genes.csv --foldchange /data/MicrobiomeCore/analysis/KFAN01_20220809/Transnet_mw_sp/merged/comp/C1-C4/p1_cp0.15_cfdr0.15/merged_mw_FolChMedian_merged-parallel-output.csv --indivPvalCutoff 1 --combPvalCutoff 0.05 --analysisfc Analys\ 5\  --analysiscorr Analys\ 21\  --combFDRCutoff 0.15 --output ./net_ --numbDataFromFCfile --voting
...
p <- add_argument(p, "--voting", help="use majority voting", flag=TRUE)
...
# receives one row with pvalues and corr coeffs as columns
majority = function(sdf){
    #sdf = df[1, , drop=F]
		sdf = data.frame(as.list(sdf))
		#print(sdf)
		
		pvalvec = sdf[, grep("pvalue", colnames(sdf), value=T)]
		coefvec = sdf[, grep("Coefficient", colnames(sdf), value=T)]
		...
		sign.vec = c()
		vec.for.fisher = c()
		coefvec.w.nonSigPvalIgnored = c()

		# if pval is significant, keep info else switch to NA
		for(idx in 1:length(pvalvec)){
				origpv = as.numeric(unlist(pvalvec[idx]))
				origcoef = as.numeric(unlist(coefvec[idx]))
				
				if( (!is.na(origpv)) & (origpv < 0.1) ){
							vec.for.fisher = c(vec.for.fisher, origpv)
							sign.vec = c(sign.vec, sign(origcoef))
							coefvec.w.nonSigPvalIgnored = c(coefvec.w.nonSigPvalIgnored, origcoef)
				} else{
							vec.for.fisher = c(vec.for.fisher, NA)
							sign.vec = c(sign.vec, NA)
							coefvec.w.nonSigPvalIgnored = c(coefvec.w.nonSigPvalIgnored, NA)
				}
		}
		
		names(vec.for.fisher) = colnames(pvalvec)
		names(sign.vec) = colnames(coefvec)
		names(coefvec.w.nonSigPvalIgnored) = colnames(coefvec)
		
		print(vec.for.fisher)
		#print(sign.vec)
		#coefvec.w.nonSigPvalIgnored = as.numeric(unlist(coefvec)) * abs(sign.vec)
		print(coefvec.w.nonSigPvalIgnored)

		if( (length(sign.vec[!is.na(sign.vec)]) > 0) &  (length(unique(sign.vec[!is.na(sign.vec)])) == 1) ){
			status = c("keep")
			names(status) = "status"
			ret = c(vec.for.fisher, coefvec.w.nonSigPvalIgnored, status)
		} else{
			status = c("skip")
			names(status) = "status"
			ret = c(vec.for.fisher, coefvec.w.nonSigPvalIgnored, status)
		...
		## return something that can be fed to the next piece of the code as is.
		return(ret)
}
...
if(voting){
		...
		##for (i in 1:nrow(df)){majority(df[i, , drop=F])}
		#ans = t(apply(df, 1, majority))
		#rownames(ans) = df[, 1]
		
		# this shouldn't matter, but doing this as an extra check to keep only what's needed
		colsToKeep = grep(search_group, colnames(outForPUC), value=T)
		print(colsToKeep)
		PvalCorr_colsToKeep = grep("pvalue|Coefficient", colsToKeep, value=T)
		print(PvalCorr_colsToKeep)
		
		df = outForPUC[, PvalCorr_colsToKeep]
		ans = t(apply(df, 1, majority))
		rownames(ans) = rownames(df)
		
		print(head(ans))

		throw = ans[(which(ans[, "status"] == "skip")), ]
		print(head(throw))
		
		outForPUC = ans[(which(ans[, "status"] == "keep")), ]
		print(head(outForPUC))

		print(nrow(ans))
		print(nrow(throw))
		print(nrow(outForPUC))
