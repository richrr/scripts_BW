## specific version where landmark is done with "OS" and for deciding AE status and the KM and Cox is done using OS
...
# ml R/4.0.5
# cd /data/rodriguesrr/FMT_pittsburg/Pittsburg_Cohorts_paper/final-AE-related-analysis/landmark/sep-11-2021/G_1_4
# Rscript /data/rodriguesrr/scripts/R/OS-landmark-OS-KM.Cox-landmark-analysis.R ./ infile-G_1_4-specific-landmark-analysis.txt
...
landmark_analysis = function(file, BMT, lnd, substract){
  ...
  lm_dat = lm_dat[,c("Overall_Survival_days","Death_Event","time_to_AE","landmarkDeltaA")]
  ...
  print(lm_fit)
  pdf(paste(file,lnd,"substract-landmark-days",substract,"OS-landmark-OS.KM.pdf",sep="_"), onefile=F)
  ...
  print(result.table)
  write.csv(result.table, file=paste(file,lnd,"substract-landmark-days",substract,"OS-landmark-OS.Cox-regress.csv",sep="_"), row.names=F, quote=F)
