...
biomarkerCounts <- function(biomarker, cutpoint) {
  ...
  input.table.col.names <- c( "Cutpoint", "Biomarker < Cutpoint", "Biomarker >= Cutpoint")
  input.table.row.names <- c( cutpoint, as.character(length.results[2]), as.character(length.results[3]))
  input.table <- data.frame(input.table.col.names, input.table.row.names)
  colnames(input.table) <- c("Estimate", "Result")
  write.csv(input.table, file="Input-stats.csv", row.names=F, quote=F)
...
resultTableSurvival <- function(biomarker, cutpoint, survival, event, df, category, biomarkername) {
  
  input.table <- biomarkerCounts(biomarker, cutpoint)
  print(input.table)
  
  # using x , i.e., 0 and 1 worked correctly. However, I am providing low and high so they show up in the results
  ...
  # original_levels_in_categ = levels(df$category)
  # number_of_levels_in_categ = length(levels(df$category))
  # last_categ_after_sort_is_ref = levels(df$category)[number_of_levels_in_categ]
  # df$category = relevel(df$category, ref = last_categ_after_sort_is_ref)
  # new_levels_in_categ = levels(df$category)
  #------------------------------------
  df$category = forcats::fct_rev(df$category) # reverses the order of levels and basically ends up doing the same as above
  

  res.cox <- do.call(
    coxph,
    list(formula = Surv(survival, event) ~ category, data = df)
  )
...
cutPMethod <- function(df, time, event, biomarker, minimumSamps) {
  ...
  restext <- resultTableSurvival(vector.biomarker, cutpoint, vector.survival, vector.event, category.df, category, biomarker)
  print(restext)
