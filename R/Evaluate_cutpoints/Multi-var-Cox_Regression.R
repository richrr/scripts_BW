...
# allow an argument for categorical variables .
# try to match the factor levels to that of the categorical
...
CoxMethod <- function(df, time, event, biomarker, cutpoints) {
  ...
  for(i in 1:length(cutpoints)){
    ...
    if(!is.na(cutpoints[i])){
        ...
        tmpvec = ifelse( vector.biomarker[,i] < cutpoints[i], "low", "high" )
        tmp = forcats::fct_rev(tmpvec)  # reverse the order to match Evaluate_cutpoints/EvaluateCutpoints_multiple-analyses_analysis.R
    } else{
        tmp = forcats::fct_rev(vector.biomarker[,i]) # reverse the order to match Cox_Regression_Plot_Kaplan_Meyer_for_categories_multiple-analyses.R 
    }
