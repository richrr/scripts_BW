...
# parameters
#==================================================================================================================
p <- arg_parser('Use fisher alternative for comb pvalue. Extract pairs which pass the FDR and PUC criteria across datasets for a single correlation (group)')
...
p <- add_argument(p, "--output", help="output file", default="./netw_metacor_")
...
p <- add_argument(p, "--sampSizes", help="comma separated, no spaces allowed") # required; but written as optional format so I explicitly mention the arg
...
sample_size = as.numeric(strsplit(argv$sampSizes, ",")[[1]]) # comma separated, no spaces allowed
sample_size
...
calc_PUC_at_thresholds = function(PUCoutfile, df, str='all'){
   ...
   # calc the combinedpvalue using fisher alternative:
   #print(head(interestedCoefficientData))

	 combinedPvalue = ""
	 if(argv$multicore){
	 			combinedPvalue = parApply(cl, interestedCoefficientData, 1, function(x, sample_size){
																				 # to get rid of the "Error in lm.fit(x, y, offset = offset, singular.ok = singular.ok, ...) : NA/NaN/Inf in 'y'"
																				 x = replace(x, x==1, 0.9999999)
																				 x = replace(x, x==-1, -0.9999999)

																				 res = metacor(cor = x, n = sample_size, sm = "ZCOR")
																				 res$pval.fixed
																		}, sample_size)
	 } else{
	 combinedPvalue = apply(interestedCoefficientData, 1, function(x){
																		# to get rid of the "Error in lm.fit(x, y, offset = offset, singular.ok = singular.ok, ...) : NA/NaN/Inf in 'y'"
																		x = replace(x, x==1, 0.9999999)
																		x = replace(x, x==-1, -0.9999999)
